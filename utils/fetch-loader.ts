      ...
      .then(
        (response: Response): Promise<string | ArrayBuffer> => {
          ...
          reader.read().then((data: { done: boolean; value: Uint8Array }) => {
            ...
            stats.boxLoaded = chunk.length;
            const moofBox = findBox(chunk, ['moof']);
            moofBox.forEach(function () {
              stats.startTimeData.push({
                timestamp: self.performance.now(),
                len: chunk.length,
              });
            });
            const mdatBox = findBox(chunk, ['mdat']);
            mdatBox.forEach(function () {
              stats.endTimeData.push({
                timestamp: self.performance.now(),
                len: chunk.length,
              });
            });
